...
imgTarget = cv2.imread(
    "C:/Users/khanh/Desktop/IITM/DSA_ry/D06ss/Agu/Proj/TargetImage.jpg"
)
# Check if image is loaded
if imgTarget is None:
    print("Error: Target image not loaded. Check the file path.")
    exit()
    ...
    for m, n in matches:
        if m.distance < 0.75 * n.distance:
            ...
            imgFeatures = cv2.drawMatches(
                imgTarget, kp1, imgWebcam, kp2, good, None, flags=2
            )
        if len(good) > 20:
            detection = True
            ...
            if (
                len(srcPts) >= 4 and len(dstPts) >= 4
            ):  # Ensure minimum points for homography
        ...
        if "matrix" in locals() and matrix is not None:
            ...
            imgStacked = stackImages(
                ([imgWebcam, imgVideo, imgTarget], [imgFeatures, imgWarp, imgAug]), 0.5
            )

        else:
            imgStacked = stackImages(
                ([imgWebcam, imgVideo, imgTarget], [imgFeatures, imgVideo, imgWebcam]),
                0.5,
            )
